...
describe DockerStreamLogger do
  ...
  context "Pulling an image from the registry" do
    ...
    it "Prints a message before pulling the image" do
      expect(output.first).to eq "Pulling from assemblyline/builder-frontendjs:0.12.0"
    end
    ...
    it "Prints a message when the download is done" do
      expect(output.last).to eq "Status: Downloaded newer image for quay.io/assemblyline/builder-frontendjs:0.12.0"
    end
  end
  ...
  context "Building a Dockerfile where the image has allready been pulled" do
    ...
    it "formats the status info correctly" do
      expect(output[1]).to eq "Pulling from assemblyline/ubuntu:14.04.2"
      expect(output[2]).to eq "Digest: sha256:5ae07f4ea4e0adf0f86e9d703e2e783d1bbf4d76aaf06092fb1bce3e625ad7bf"
      expect(output[3]).to eq "Status: Image is up to date for quay.io/assemblyline/ubuntu:14.04.2"
    end
  end
  ...
  context "Building a Dockerfile where the image has not allready been pulled" do
    ...
    it "formats a progress bar when downloading the image" do
      ...
      expect(output[5]).to eq "Digest: sha256:5ae07f4ea4e0adf0f86e9d703e2e783d1bbf4d76aaf06092fb1bce3e625ad7bf"
      expect(output[6]).to eq "Status: Downloaded newer image for quay.io/assemblyline/ubuntu:14.04.2"
    end
  end
